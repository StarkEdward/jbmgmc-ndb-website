...
export default function StorageManagement() {
  ...
  const confirmDelete = (filesToDelete: string[]) => {
    setDeleteModal({ isOpen: true, filesToDelete })
  }

  const executeDelete = async () => {
    const filesToDelete = deleteModal.filesToDelete
    setDeleteModal({ isOpen: false, filesToDelete: [] })
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white dark:bg-slate-900 shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            <div className="p-6">
              ...
              <p className="text-sm text-center text-slate-500 dark:text-slate-400 mb-6">
                Are you sure you want to permanently delete <strong className="text-slate-700 dark:text-slate-200">{deleteModal.filesToDelete.length}</strong> file(s)? 
                This action cannot be undone and files will be permanently removed from the server.
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => setDeleteModal({ isOpen: false, filesToDelete: [] })}
                ...
                >
                  Yes, Delete {deleteModal.filesToDelete.length > 1 ? 'All' : 'It'}
                </button>
              </div>
            </div>
          </div>
